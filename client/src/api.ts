...
export async function forgotPassword(email: string): Promise<APIResponse> {
  return axios
    .post(`${BACKEND_BASE_URL}/forgotPassword`, { email })
    .then((response) => {
      console.log(response.data);
...
export async function signIn(
  ...
  return axios
    .post(`${BACKEND_BASE_URL}/login`, { email, password })
    .then(async (response) => {
    ...
    .catch((error) => {
      return {
        error: true,
        message: getError(error.response.data.errors).message,
      };
    });
}
...
export async function updatePassword(
  ...
  return axios
    .patch(
      `${BACKEND_BASE_URL}/updatePassword`,
      {
...
export async function updateProfile(patchData: {}): Promise<APIResponse> {
  let token = await getToken();
  return await axios
    .patch(`${BACKEND_BASE_URL}/profile`, patchData, {
      headers: { Authorization: `Bearer ${token}` },
    })
    .then(async (response) => {
      console.log(response.data);
