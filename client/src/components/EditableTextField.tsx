...
export default function EditableTextField(input: EditableTextFieldParams) {
  ...
  const openDrawer = () => {
    ...
    else bottomDrawerRef.current?.open(150);
  };
  const SaveProfile = async () => {
    if (token === null) {
      ...
      } else if (input.fieldName === "Department") {
        patchData = { department: newValue };
      } else {
      ...
      if (error) {
        ...
        Toast.show({
          type: "success",
          text1: "Saved Profile Successfully",
          position: "bottom",
        });
