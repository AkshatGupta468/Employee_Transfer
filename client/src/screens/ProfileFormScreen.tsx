...
export default function ProfileFormScreen({ route, navigation }: ScreenProps) {
  ...
  const [selectedCurrentLocation, setSelectedCurrentLocation] =
    useState<string>();
  const [selectedDeparment, setSelectedDeprtment] = useState<string>();
  ...
  const saveProfile = async () => {
    let { error, message } = await saveProfileAPI(
      name,
    ...
    <View style={AppStyles.container}>
      <PaperProvider>
        ...
        <TextInput
          ...
          multiline={false}
          activeOutlineColor={colors["dark-grey"]}
        ...
        <SelectList
          ...
          dropdownStyles={{ width: 250, alignSelf: "center" }}
          inputStyles={{ color: colors["dark"], fontSize: 16 }}
          setSelected={setSelectedDeprtment}
          data={DeptData}
